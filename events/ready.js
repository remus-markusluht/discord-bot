...
//
    ...
    await console.log("――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――");
    await console.log("                                                                          ");
    if(config.LicenseKey) await console.log(`${color.green.bold(`Example Tickets v${botVersion.version} is now Online!`)} (${color.gray(`${config.LicenseKey.slice(0, -10)}`)})`);
    if(!config.LicenseKey) await console.log(`${color.green.bold(`Example Tickets v${botVersion.version} is now Online! `)}`);
    await console.log("                                                                          ");
    await console.log(`• Join our discord server for support, ${color.cyan(`discord.gg/`)}`);
    await console.log(`• By using this bot you agree to all terms located here, ${color.yellow(`discord.gg/`)}`);
    await console.log("                                                                          ");
    await console.log("――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――――");
...
if(hours > config.TicketAlert.Time) {
  ...
  let attachment = await utils.saveTranscriptAlertCmd(channel)
    ...
    logEmbed.setThumbnail(closeUserID.displayAvatarURL({ dynamic: true, size: 1024 }))
    logEmbed.setFooter({ text: `${config.Locale.totalMessagesLog} ${totalMessages}`, iconURL: `${closeUserID.displayAvatarURL({ dynamic: true, size: 1024 })}` })
    ...
    await client.tickets.delete(channel.id)
...
if(config.StripeSettings.Enabled || config.PayPalSettings.Enabled) {
  setInterval(async() => {
  ...
  if(config.StripeSettings.Enabled) {
    ...
    if(config.PayPalSettings.Enabled) {
      const filtered = client.paypalInvoices.filter(p => p.status === "DRAFT")
        filtered.forEach(async eachPayment => {
          ...
          if(channel && user && client.paypalInvoices.get(`${invoice.id}`, "status") === "paid") {
            await channel.messages.fetch(eachPayment.messageID).catch(e => {}).then(async msg => {
              ...
              const embedColor = Discord.EmbedBuilder.from(embed)
              embedColor.setColor("Green")
              await msg.edit({ embeds: [embedColor], components: [row] })
