const { Discord, ActionRowBuilder, ButtonBuilder, EmbedBuilder, InteractionType, Message, SelectMenuBuilder, ModalBuilder, TextInputBuilder } = require("discord.js");
const fs = require('fs');
...
await config.TicketButton1.SupportRoles.forEach(roleid => {
    ...
    permissionOverwriteArray.push(tempArray);
        ...
        var userIcon = interaction.user.displayAvatarURL({ dynamic: true, size: 1024 });
...
if (interaction.customId === 'confirm') {
    ...
    interaction.channel.messages.fetch(client.tickets.get(interaction.channel.id, 'closeConfirmationMsgID')).then(msg => {
        msg.delete()
    })
}
