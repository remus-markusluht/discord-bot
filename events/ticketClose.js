const { Discord, ActionRowBuilder, ButtonBuilder, EmbedBuilder, SelectMenuBuilder, Message, MessageAttachment, ModalBuilder, TextInputBuilder } = require("discord.js");
const fs = require('fs');
  ...
  await interaction.deferReply();
    ...
    async function CloseTicket() {
        ...
        let ticketCloseReason = await client.tickets.get(`${interaction.channel.id}`, "closeReason")
          ...
          logEmbed.setThumbnail(interaction.user.displayAvatarURL({ dynamic: true, size: 1024 }))
          logEmbed.setFooter({ text: `${config.Locale.totalMessagesLog} ${totalMessages}`, iconURL: `${interaction.user.displayAvatarURL({ dynamic: true, size: 1024 })}` })
...
//
      ...
      if(config.TicketSettings.CloseConfirmation && config.ArchiveTickets.Enabled) await interaction.editReply({ embeds: [delEmbed] })
...
//
      ...
      let msgID;
      if(config.TicketSettings.CloseConfirmation) await interaction.editReply({ embeds: [embed], components: [row], fetchReply: true }).then(async function(msg) { msgID = msg.id })
      if(config.TicketSettings.CloseConfirmation === false) await interaction.editReply({ embeds: [embed], components: [row], fetchReply: true }).then(async function(msg) { msgID = msg.id })
      ...
      if(config.ArchiveTickets.Enabled === true && interaction.customId === 'deleteTicket') {
        ...
        let row = new ActionRowBuilder().addComponents(confirmButton, cancelButton);
        ...
        .setTimestamp()
        if(config.TicketSettings.CloseConfirmation === true && interaction.customId !== 'confirm') await interaction.editReply({ embeds: [embed], components: [row], fetchReply: true }).then(async function(msg) { msgID = msg.id })
        if(config.TicketSettings.CloseConfirmation === true) await client.tickets.set(interaction.channel.id, msgID, "closeConfirmationMsgID");
        if(config.TicketSettings.CloseConfirmation === false) return ArchiveTicket()
          ...
          CloseTicket()
      } else if(config.TicketSettings.CloseConfirmation === true && config.ArchiveTickets.Enabled === false && interaction.customId !== 'confirm') {
        ...
        .setDescription(confirmCloseLocale)
        .setTimestamp()
        await interaction.editReply({ embeds: [embed], components: [row] }).then(async function(msg) { msgID = msg.id })
        await client.tickets.set(interaction.channel.id, msgID, "closeConfirmationMsgID");

      }

// Confirm ticket closure
if (interaction.customId === 'confirm') {
  if(config.ArchiveTickets.Enabled) ArchiveTicket()
  if(config.ArchiveTickets.Enabled === false) CloseTicket()
}
